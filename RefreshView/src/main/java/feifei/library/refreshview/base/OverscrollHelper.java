...
*            - true if this scroll operation is the result of a touch
	 */
	public static void overScrollBy(final RefreshBase<?> view,
	...
	public static void overScrollBy(final RefreshBase<?> view,
		...
		if (view.isPullToRefreshOverScrollEnabled() && !view.isRefreshing()) {
			final RefreshBase.Mode mode = view.getMode();
				...
				if (newScrollValue < (0 - fuzzyThreshold)) {
					...
					if (mode.showHeaderLoadingLayout()) {
						...
						if (currentScrollValue == 0) {
							view.setState(RefreshBase.State.OVERSCROLLING);
						}
					...
					view.setState(RefreshBase.State.RESET);
				}
			} else if (isTouchEvent && RefreshBase.State.OVERSCROLLING == view.getState()) {
