...
import feifei.library.refreshview.base.IndicatorLayout;


public abstract class RefreshAdapterViewBase<T extends AbsListView> extends
		RefreshBase<T> implements OnScrollListener {
	...
	public RefreshAdapterViewBase(Context context) {
		super(context);
		mRefreshableView.setOnScrollListener(this);
	}

	public RefreshAdapterViewBase(Context context, AttributeSet attrs) {
		super(context, attrs);
		mRefreshableView.setOnScrollListener(this);
	}

	public RefreshAdapterViewBase(Context context, Mode mode) {
		super(context, mode);
		mRefreshableView.setOnScrollListener(this);
	}
	 ...
	 * {@link RefreshBase#isPullToRefreshOverScrollEnabled()
	...
	public final void onScroll(final AbsListView view,
		...
		/**
		 * Set high with keybroad dipear;
		 */
		// TODO 设置高度，解决了键盘问题，但是需要伸缩高度的没有解决，加一个变量判断，为true则什么也不需要判断
		setLayoutParams(new LinearLayout.LayoutParams(
				LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
		Log.d("文章列表", "ListView:" + getHeight() + "");
		//		if (!needChange) {
		//			int tmpHigh = getHeight();
		//			if (tmpHigh > 0) {
		//				if (MHigh < tmpHigh) {
		//					MHigh = tmpHigh;
		//				} else {
		//					ViewGroup.LayoutParams viewgroupparams = getLayoutParams();
		//					viewgroupparams.height = MHigh;
	...
	/**
	 * Pass-through method for {@link RefreshBase#getRefreshableView()
	 * getRefreshableView()}.
