...
public class SwipeDismissListener implements View.OnTouchListener {
	...
	public SwipeDismissListener(View view, OnDismissCallback callback) {
		final ViewConfiguration mViewConfiguration = ViewConfiguration.get(view
				.getContext());
	...
	public boolean onTouch(View view, MotionEvent motionEvent) {
		...
		case MotionEvent.ACTION_UP:
			...
			float deltaXActionUp = motionEvent.getRawX()
					- mActionDownXCoordinate;
			mVelocityTracker.addMovement(motionEvent);
		...
		case MotionEvent.ACTION_MOVE:
			...
			mVelocityTracker.addMovement(motionEvent);
			float deltaXActionMove = motionEvent.getRawX()
					- mActionDownXCoordinate;
			...
			if (isSwiping) {
				...
				mView.setAlpha(Math.max(
						0f,
						Math.min(1f, 1f - 2f * Math.abs(deltaXActionMove)
								/ mViewWidth)));
				return true;
			}
			break;
