...
public class LinkedNotebook implements TBase<LinkedNotebook>, java.io.Serializable, Cloneable {
	...
	private static final TField SHARE_NAME_FIELD_DESC = new TField("shareName", TType.STRING,
			(short) 2);
	...
	public int compareTo(LinkedNotebook other) {
		...
		LinkedNotebook typedOther = other;
		...
		if (isSetShareName()) {
			lastComparison = TBaseHelper.compareTo(this.shareName, typedOther.shareName);
			if (lastComparison != 0) {
				return lastComparison;
			}
		}
		...
		if (isSetShardId()) {
			lastComparison = TBaseHelper.compareTo(this.shardId, typedOther.shardId);
			if (lastComparison != 0) {
				return lastComparison;
			}
		}
