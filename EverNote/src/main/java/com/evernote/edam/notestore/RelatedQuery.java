...
public class RelatedQuery implements TBase<RelatedQuery>, java.io.Serializable, Cloneable {
	...
	private static final TField NOTE_GUID_FIELD_DESC = new TField("noteGuid", TType.STRING,
			(short) 1);
	private static final TField PLAIN_TEXT_FIELD_DESC = new TField("plainText", TType.STRING,
			(short) 2);
	private static final TField FILTER_FIELD_DESC = new TField("filter", TType.STRUCT, (short) 3);
	private static final TField REFERENCE_URI_FIELD_DESC = new TField("referenceUri", TType.STRING,
			(short) 4);
	...
	public int compareTo(RelatedQuery other) {
		...
		if (isSetPlainText()) {
			lastComparison = TBaseHelper.compareTo(this.plainText, typedOther.plainText);
			if (lastComparison != 0) {
				return lastComparison;
			}
		}
		...
		lastComparison = Boolean.valueOf(isSetReferenceUri()).compareTo(
				typedOther.isSetReferenceUri());
		if (lastComparison != 0) {
			return lastComparison;
		}
		if (isSetReferenceUri()) {
			lastComparison = TBaseHelper.compareTo(this.referenceUri, typedOther.referenceUri);
			if (lastComparison != 0) {
				return lastComparison;
			}
		}
