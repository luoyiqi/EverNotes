...
public class NoteCollectionCounts implements TBase<NoteCollectionCounts>, java.io.Serializable,
		Cloneable {
	private static final TStruct STRUCT_DESC = new TStruct("NoteCollectionCounts");

	private static final TField NOTEBOOK_COUNTS_FIELD_DESC = new TField("notebookCounts",
			TType.MAP, (short) 1);
	private static final TField TAG_COUNTS_FIELD_DESC = new TField("tagCounts", TType.MAP,
			(short) 2);
	private static final TField TRASH_COUNT_FIELD_DESC = new TField("trashCount", TType.I32,
			(short) 3);
	...
	public NoteCollectionCounts(NoteCollectionCounts other) {
		...
		if (other.isSetNotebookCounts()) {
			...
			for (Map.Entry<String, Integer> other_element : other.notebookCounts.entrySet()) {
				...
				__this__notebookCounts.put(__this__notebookCounts_copy_key,
						__this__notebookCounts_copy_value);
			}
	...
	public int compareTo(NoteCollectionCounts other) {
		...
		NoteCollectionCounts typedOther = other;

		lastComparison = Boolean.valueOf(isSetNotebookCounts()).compareTo(
				typedOther.isSetNotebookCounts());
		if (lastComparison != 0) {
			return lastComparison;
		}
		if (isSetNotebookCounts()) {
			lastComparison = TBaseHelper.compareTo(this.notebookCounts, typedOther.notebookCounts);
			if (lastComparison != 0) {
				return lastComparison;
			}
		}
		...
		if (isSetTagCounts()) {
			lastComparison = TBaseHelper.compareTo(this.tagCounts, typedOther.tagCounts);
			if (lastComparison != 0) {
				return lastComparison;
			}
		}
		...
		if (isSetTrashCount()) {
			lastComparison = TBaseHelper.compareTo(this.trashCount, typedOther.trashCount);
			if (lastComparison != 0) {
				return lastComparison;
			}
		}
	...
	public void write(TProtocol oprot) throws TException {
		...
		if (this.notebookCounts != null) {
			if (isSetNotebookCounts()) {
				...
				{
					oprot.writeMapBegin(new TMap(TType.STRING, TType.I32, this.notebookCounts
							.size()));
