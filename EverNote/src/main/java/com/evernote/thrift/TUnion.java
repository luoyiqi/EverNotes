...
public abstract class TUnion<T extends TUnion, F extends TFieldIdEnum> implements
		TReflectionBase<T, F> {
	...
	public Object getFieldValue(F fieldId) {
		if (fieldId != setField_) {
			throw new IllegalArgumentException("Cannot get the value of field " + fieldId
					+ " because union's set field is " + setField_);
		}
	...
	public void read(TProtocol iprot) throws TException {
		...
		iprot.readFieldEnd();
		// this is so that we will eat the stop byte. we could put a check here
		// to
		// make sure that it actually *is* the stop byte, but it's faster to do
		// it
		// this way.
