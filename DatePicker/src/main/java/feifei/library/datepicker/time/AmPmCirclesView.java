...
public class AmPmCirclesView extends View {
	...
	public int getIsTouchingAmOrPm(float xCoord, float yCoord) {
		...
		int distanceToAmCenter = (int) Math.sqrt((xCoord - mAmXCenter)
				* (xCoord - mAmXCenter) + squaredYDistance);
		if (distanceToAmCenter <= mAmPmCircleRadius) {
			return AM;
		}

		int distanceToPmCenter = (int) Math.sqrt((xCoord - mPmXCenter)
				* (xCoord - mPmXCenter) + squaredYDistance);
	...
	public void onDraw(Canvas canvas) {
		...
		int textYCenter = mAmPmYCenter
				- (int) (mPaint.descent() + mPaint.ascent()) / 2;
